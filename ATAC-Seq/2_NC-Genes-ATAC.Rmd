...
register(BPPARAM = MulticoreParam(progressbar = T, workers = 60, stop.on.error = F))
...
strong_correlates <- TC_NC_Enriched_TF_enhancers[TC_NC_Enriched_TF_enhancers$spearman > 0.8 | TC_NC_Enriched_TF_enhancers$spearman < -0.8,]
...
H3K27Ac_corr <- dba.peakset(H3K27Ac_corr,peaks=strong_correlates.gr,
                            sampID="NF_1",tissue="NF",factor="CR",condition="H3K27AC",replicate=1, bamReads="/data/Austin/workdir/NC_Timecourse_Data/Other_CR_Sox2-Oct4Proj/BAM/11046_10331_104544_HVGL5BGXB_HH9WE_H3K27Ac_1_GTGAAACG_R1_toGalGal6.bam")
H3K27Ac_corr <- dba.peakset(H3K27Ac_corr,peaks=strong_correlates.gr,
                            sampID="NF_2",tissue="NF",factor="CR",condition="H3K27AC",replicate=2, bamReads="/data/Austin/workdir/NC_Timecourse_Data/Other_CR_Sox2-Oct4Proj/BAM/11046_10331_104545_HVGL5BGXB_HH9WE_H3K27Ac_2_GTGGCCTT_R1_toGalGal6.bam")
...
Corr_group$group <- ifelse(Corr_group$spearman > 0.8, "Positive","Negative")

rld_df$Corr_group <- qdapTools::lookup(rownames(rld_df), Corr_group[,c("PeakID","group")])
rld_df$PeakID <- rownames(rld_df)
rld_df_melt <- reshape2::melt(rld_df)

ggplot(rld_df_melt, mapping = aes(x = Corr_group, y = value, color = Corr_group)) + geom_boxplot() + stat_compare_means()
ggsave(filename = "Exports/Positive_vs_Negative_strong_correlation_H3K27Ac_Levels.eps", width = 5, height = 7)
```
