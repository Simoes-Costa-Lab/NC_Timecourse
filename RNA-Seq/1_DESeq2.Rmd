...
library(viridis)
library(qdapTools)
register(MulticoreParam(4)) # Set this to number of CPUs.
```
...
DESeq2_Analyze <- function(timepoint, dds, genes.universe = genes.universe, Padj = 0.1, L2fc = 1, testMF = TRUE, testBP = TRUE, plotVolc = TRUE, labelPadj = 0.001) {
  ...
  if (plotVolc == TRUE){
      ...
      g <- ggplot(data = restest3, aes(x = log2FoldChange, y = -log10(padj))) +
        ...
        geom_text_repel(data=subset(restest2, padj< labelPadj & 
                                      abs(log2FoldChange) >=L2fc & 
                                      Symbol %in% Multipotency_set),aes(label = Symbol), box.padding = 0.2) +
...
# Import multipotency gene sets
Mueller <- read.delim(file = "Imports/Mueller_Plurinet.grp", skip = 1)
Wong <- read.delim(file = "Imports/Wong_stemcellcore.grp", skip = 1)
Multipotency_set <- unique(c(as.character(Mueller[,1]), as.character(Wong[,1])))

DESeq2_Analyze("6", dds, genes.universe = genes.universe, Padj = 0.05, L2fc = 1.5, testMF = TRUE, testBP = TRUE, plotVolc = TRUE, labelPadj = 0.001)
