...
```{r}
colors <- colorRampPalette(c("#212CC4","#FFFFFF","#C42126"))(250)

All_Enriched_Genes <- ddsTC_Condition_WE_vc_NC_All %>% dplyr::filter(log2FoldChange > 0.5 & padj < 0.05)
enriched_1 <- Annotated_Rlog_Positive_Averages[All_Enriched_Genes$Chick_ENSEMBL,2:7]

enriched <- pheatmap(enriched_1, scale = "row", cluster_cols = F,
                     show_rownames = F, cellwidth = 20, color = colors,
                     filename = "./Exports/ddsTC_All_Enriched_Genes.pdf", width = 6, height = 8)
