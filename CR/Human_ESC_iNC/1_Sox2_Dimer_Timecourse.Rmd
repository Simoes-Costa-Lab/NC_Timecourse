...
## Looking at H1 to iNC induction from TFAP2A Overexpression
How does Sox2 CUT&RUN change from H1 to H1+TFAP2A? Gain of Sox2 binding at NC-Specific Dimer motifs?

```{r}
library(DiffBind)
Candidate_peaks <- c(D0_Sox2_Peaks, D5_Sox2_Peaks)
# WT Sox2
Sox2_Signal_OE <- dba.peakset(NULL,peaks=Candidate_peaks,
                      sampID="ESC_D0_1",tissue="ESC",
                      factor="WT",condition="D0",
                      replicate=1,
                      bamReads="/data/Austin/workdir/NC_Timecourse_Data/Human_ESC_iNC_CR/BAM/H1_std_Sox2hg38_filt.bam")

Sox2_Signal_OE <- dba.peakset(Sox2_Signal_OE,peaks=Candidate_peaks,
                      sampID="ESC_D0_2",tissue="ESC",
                      factor="WT",condition="D0",
                      replicate=2,
                      bamReads="/data/Austin/workdir/NC_Timecourse_Data/Human_ESC_iNC_CR/BAM/H1_Sox2_auto_ACAGTG_hg38_filt.bam")
...
Sox2_Signal_OE <- dba.count(Sox2_Signal_OE, bParallel = T)
dba.plotPCA(Sox2_Signal_OE, attributes = DBA_CONDITION)
Sox2_Signal_OE <- dba.contrast(Sox2_Signal_OE, contrast = c("Tissue","ESC","NC"))
Sox2_Signal_OE <- dba.analyze(Sox2_Signal_OE)
Sox2_res <- as.data.frame(dba.report(Sox2_Signal_OE))
```
