...
human_ATAC_avg_subset <- human_ATAC_avg[rownames(human_ATAC_avg) %in% All_Unique_PeakIDs,]

ATAC_clusters = pheatmap(human_ATAC_avg_subset,
                         cluster_cols = F,
                         scale = "row",
                         show_rownames = F,
                         show_colnames = T,
                         cluster_rows = T,
                         cutree_rows = 6, color=colorRampPalette(colors=c("#0797B3","#FFFFFF","#4FA14E"))(250))

#Re-order original data (genes) to match ordering in heatmap (top-to-bottom)
rownames(human_ATAC_avg_subset[ATAC_clusters$tree_row[["order"]],])
#6 groups
...
pheatmap(human_ATAC_avg_subset, annotation_row = row_annotation, 
